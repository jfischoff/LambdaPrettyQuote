...
--gen_env :: (Eq a, Eq s) => Gen a -> Gen s -> Gen (Env a s)
--gen_env x y = gen_with_env arb_env x y $ const null_const_gen
...
{-
arb_env :: (Eq a, Eq s) => EnvGen a s c (Env a s)
arb_env = do 
    i <- get_size
    foldl1' (>>) $ take i (cycle [arb_extend])
    get_env
    
arb_extend :: (Eq a, Eq s) => EnvGen a s c ()
arb_extend = join $ liftM2 extend arb_s arb_type

arb_extend' :: (Eq a, Eq s) => s -> EnvGen a s c ()
arb_extend' s = join $ liftM (extend s) arb_type
-}
...
arb_existing_type :: (Eq a, Eq s) => EnvGen a s c (Type a)
arb_existing_type = do 
    env <- get_env
    let ts = map snd env
    lift $ elements ts
